...
const getBooking = asyncHandler(async (req, res) => {
  ...
  if (!isCurrentAuthUser(req.user, userRole.roletype, reservation))
...
const getBookedDates = asyncHandler(async (req, res) => {
  let bookedDates = [];
  const x = (await Reservation.find()).filter((x) =>
    bookedDates.push(x.reservation_date)
  );
...
const getAllBookingsByUserId = asyncHandler(async (req, res) => {
  ...
  const isAuthorirzed =
    ...
    userRole.roletype === defaultRolesAndUsers.ADMIN;
  if (!isAuthorirzed) {
...
const createBooking = asyncHandler(async (req, res) => {
  try {
    ...
    if (!foundUser) {
      return res
        .status(404)
        .json({ error: "No User found with this email/username" });
    }
    ...
    let booking_reference =
      foundUser.username
        ...
        .toUpperCase() +
      "" +
      moment(date).format("DMYYhmmss").toString();
    let bookingDate = moment(new Date(req.body.reservation_date)).format(
      "YYYY-MM-DDT00:00:00"
    );
    ...
    if (reservation) {
      ...
      let new_booking = await Reservation.findOne({
        reservation_date: bookingDate,
      });
...
const updateBooking = asyncHandler(async (req, res) => {
  try {
    ...
    if (
      (await Reservation.find({ reservation_date: bookingDate })).filter(
        (x) => x.id !== req.params.id
      ).length > 0
    ) {
      return res
        .status(401)
        .json({ message: "Reservation date is not available" });
    } else {
      await Reservation.findByIdAndUpdate(req.params.id, reservationData, {
        new: true,
      });
    }
...
const isCurrentAuthUser = (user, roletype, reservation) => {
  return (
    roletype === defaultRolesAndUsers.ADMIN ||
    reservation.user._id.toString() === user._id.toString()
  );
};
...
module.exports = {
  ...
  deleteBooking,
  getBookedDates,
  getBookingsBySearchQuery,
  getBookingsByUserSearchQuery,
};
